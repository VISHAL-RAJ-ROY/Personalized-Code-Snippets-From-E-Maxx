...
void err(istream_iterator<string> it) {nl;} template<typename T, typename... Args> void err(istream_iterator<string> it, T a, Args... args) { cout << *it << " = " << a << " | "; err(++it, args...);}
...
7) vcd                               |    i3 = tuple<int,int,int> |   int/li/pair/tuple -> Anything...pair/tuple<int/li,int/li>
...
void color_segments( vi & v, vi3 & Q ) {
    int ct = 0;
    int n = sz(v);
    UFDS uf(n);
    vi end(n); 
    ...
    FOR(i,sz(Q),0) {
        ...
        error(l,r);
        watch(v);
        for( int u = end[uf.find(l)]; u <= r;  ) {
            if( !v[u] ) {
                error(u);
                v[u] = c; ct++;
                int uu;
                if( u+1 < n ) {    
                    int y = uf.unite((uu=uf.find(u+1)),u);
                    u = end[y] = end[uu]; error(y,uu,end[uu]);
                }
            }
            else 
                break;
